...
<script>
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {label: '实还日期', prop: 'actualRepayDate'},
          {label: '实还本金', prop: 'actualPrincipal'},
          {label: '实还利息', prop: 'actualInterest'},
          {label: '实还管理费', prop: 'actualManageFee'},
          {label: '实还逾期罚息', prop: 'actualDelayInterest'},
          {label: '实还逾期管理费', prop: 'actualDelayManage'},
          {label: '违约金', prop: 'liquidatedDamages'},
          {label: '实还金额', prop: 'actualAmount'},
        ]
      }
    },
      computed:{
        items() {
          ...
          },
          {
            ...
            },
            {
              type: 'dataTime',
              dataTime: {
                start: {label: '实还日期开始', key: 'actualRepayDateStart', placeholder: ''},
                end: {label: '实还日期结束', key: 'actualRepayDateEnd', placeholder: ''}
              }
            },
            {
              type: 'select',
              ...
              key: 'repaid',
              placeholder: '',
              options: this.repaidTypeOptions,
            },
          {
            type: 'buttonGroup',
            buttons: [
          ...
          },
            {
              ...
              buttons: [
                {label: '导出',  function: this.exportShoudlExcel, disabled: this.exportLoading, hidden: !this.$hasButton('loan-paid-export')}
              ]
            }
          ]
        }
    ...
    created() {
      ...
      this.repaidTypeOptions = this.$store.state.enums.RepaidType.filter(ele => {
        return ele.value === '1' || ele.value === '4'
      })
    }
  }
</script>
